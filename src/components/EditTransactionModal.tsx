...
import { useTransaction, useUpdateTransaction, useSoftDeleteTransaction } from '../hooks/useTransactions'
import { useState, useEffect } from 'react'

export function EditTransactionModal() {
  const navigate = useNavigate()
  const search = useSearch({ from: '/' })
  const isOpen = search.modal === 'edit' && !!search.id
  const transactionId = search.id || ''
  const [isSubmitting, setIsSubmitting] = useState(false)

  const { data: transaction, isLoading } = useTransaction(transactionId)
  const updateTransaction = useUpdateTransaction()
  const deleteTransaction = useSoftDeleteTransaction()
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="px-4 py-3 bg-red-500/10 text-red-400 border border-red-500/20 rounded-md font-medium hover:bg-red-500/20 transition-colors flex items-center gap-2"
                >
